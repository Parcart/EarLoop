...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate V3 semantic-active Pair Generator against previous strategies."
    )
    ...
    parser.add_argument("--prefix", type=str, default="v3_semantic_active")
...
def main() -> None:
    ...
    if len(archetype_sessions) > 0:
        ...
        if "main_archetype" in archetype_sessions.columns:
            ...
            v3_vs_v21_by_archetype.to_csv(metrics_dir / f"{args.prefix}_v3_vs_v21_by_archetype.csv", index=False)
            v3_vs_v21_win_rates.to_csv(metrics_dir / f"{args.prefix}_v3_vs_v21_win_rates_by_archetype.csv", index=False)
