...
def _display_strategy(strategy: str) -> str:
    return V51_DISPLAY_NAMES.get(strategy, strategy)
...
def plot_convergence_archetype(curves: dict[str, dict[str, np.ndarray]]) -> None:
    ...
    for strategy in STRATEGY_ORDER:
        ...
        ax.fill_between(steps, mean - std, mean + std, alpha=0.11)
...
def plot_archetype_v5_vs_v51(summary: pd.DataFrame) -> None:
    df = add_display_columns(summary)
...
def plot_v51_pair_quality(steps: pd.DataFrame) -> None:
    df = steps[(steps["target_mode"] == "archetype8d") & (steps["strategy"] == V51_STRATEGY_NAME)].copy()
    if df.empty:
        return
...
def plot_source_usage(source_usage: pd.DataFrame) -> None:
    df = source_usage[(source_usage["target_mode"] == "archetype8d") & (source_usage["strategy"] == V51_STRATEGY_NAME)].copy()
    if df.empty:
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run V5.1 conservative anchor-challenger experiment.")
    ...
    plot_archetype_v5_vs_v51(summary)
    plot_v51_pair_quality(steps)
    plot_source_usage(source_usage)

    print("Saved V5.1 results to:", METRICS_DIR)
