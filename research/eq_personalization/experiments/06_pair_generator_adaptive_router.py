...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate V3.2 adaptive router Pair Generator against V3 and baselines."
    )
    ...
    parser.add_argument("--prefix", type=str, default="v32_adaptive_router")
...
def main() -> None:
    ...
    # Direct V3.2 vs V3/V3.1 comparisons by target mode.
    router_vs_v3_target = compare_two_strategies_by_group(
        target_mode_summary,
    ...
    router_vs_candidate_target = compare_two_strategies_by_group(
        target_mode_summary,
        group_col="target_mode",
    ...
    router_vs_v3_target.to_csv(metrics_dir / f"{args.prefix}_router_vs_v3_by_target_mode.csv", index=False)
    router_vs_candidate_target.to_csv(metrics_dir / f"{args.prefix}_router_vs_candidate_by_target_mode.csv", index=False)
    ...
    if len(archetype_sessions) > 0:
        ...
        if "main_archetype" in archetype_sessions.columns:
            ...
            router_vs_v3_by_archetype.to_csv(metrics_dir / f"{args.prefix}_router_vs_v3_by_archetype.csv", index=False)
            router_vs_candidate_by_archetype.to_csv(metrics_dir / f"{args.prefix}_router_vs_candidate_by_archetype.csv", index=False)
            router_vs_v3_win_rates.to_csv(metrics_dir / f"{args.prefix}_router_vs_v3_win_rates_by_archetype.csv", index=False)
            router_vs_candidate_win_rates.to_csv(metrics_dir / f"{args.prefix}_router_vs_candidate_win_rates_by_archetype.csv", index=False)

    print("Strategy summary:")
    print(strategy_summary)
    print("\nWin rates vs random:")
    print(win_rates)
    print("\nWinners by target mode:")
    print(target_mode_winners)
