...
def _display_strategy(strategy: str) -> str:
    return V52_DISPLAY_NAMES.get(strategy, strategy)


def add_display_columns(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    out["target_mode_display"] = out["target_mode"].map(TARGET_MODE_DISPLAY_NAMES).fillna(out["target_mode"])
    out["strategy_display"] = out["strategy"].map(V52_DISPLAY_NAMES).fillna(out["strategy"])
    return out
...
def plot_mean_final_distance(summary: pd.DataFrame) -> None:
    ...
    plt.savefig(FIG_DIR / "article_v52_mean_final_distance_by_target.png", dpi=180, bbox_inches="tight")
    plt.close(fig)
...
def plot_convergence_archetype(curves: dict[str, dict[str, np.ndarray]]) -> None:
    ...
    plt.tight_layout()
    FIG_DIR.mkdir(parents=True, exist_ok=True)
    plt.savefig(FIG_DIR / "article_v52_convergence_archetype8d.png", dpi=180, bbox_inches="tight")
    plt.close(fig)
...
def plot_archetype_summary(summary: pd.DataFrame) -> None:
    ...
    plt.tight_layout()
    FIG_DIR.mkdir(parents=True, exist_ok=True)
    plt.savefig(FIG_DIR / "article_v52_archetype8d_final_distance.png", dpi=180, bbox_inches="tight")
    plt.close(fig)


def plot_v52_steps_used(summary: pd.DataFrame) -> None:
    df = add_display_columns(summary)
    df = df[(df["target_mode"] == "archetype8d") & (df["strategy"].isin(["semantic_active_v21", V52_STRATEGY_NAME]))].copy()
    df["strategy"] = pd.Categorical(df["strategy"], ["semantic_active_v21", V52_STRATEGY_NAME], ordered=True)
    df = df.sort_values("strategy")
    ...
    plt.tight_layout()
    FIG_DIR.mkdir(parents=True, exist_ok=True)
    plt.savefig(FIG_DIR / "article_v52_used_steps_archetype8d.png", dpi=180, bbox_inches="tight")
    plt.close(fig)


def plot_v52_pair_quality(steps: pd.DataFrame) -> None:
    df = steps[(steps["target_mode"] == "archetype8d") & (steps["strategy"] == V52_STRATEGY_NAME)].copy()
    if df.empty:
        return
    ...
    plt.tight_layout()
    FIG_DIR.mkdir(parents=True, exist_ok=True)
    plt.savefig(FIG_DIR / "article_v52_pair_quality_archetype8d.png", dpi=180, bbox_inches="tight")
    plt.close(fig)


def plot_source_usage(source_usage: pd.DataFrame) -> None:
    df = source_usage[(source_usage["target_mode"] == "archetype8d") & (source_usage["strategy"] == V52_STRATEGY_NAME)].copy()
    if df.empty:
        return
    ...
    plt.tight_layout()
    FIG_DIR.mkdir(parents=True, exist_ok=True)
    plt.savefig(FIG_DIR / "article_v52_pair_source_usage_archetype8d.png", dpi=180, bbox_inches="tight")
    plt.close(fig)


def main() -> None:
    parser = argparse.ArgumentParser(description="Run V5.2 semantic backbone + confirmation experiment.")
    ...
    summary.to_csv(TABLE_DIR / "article_v52_semantic_backbone_confirmation_summary.csv", index=False)
    win_rates.to_csv(TABLE_DIR / "article_v52_semantic_backbone_confirmation_win_rates.csv", index=False)
    source_usage.to_csv(TABLE_DIR / "article_v52_semantic_backbone_confirmation_source_usage.csv", index=False)

    plot_mean_final_distance(summary)
    plot_convergence_archetype(curves)
    plot_archetype_summary(summary)
    plot_v52_steps_used(summary)
    plot_v52_pair_quality(steps)
    plot_source_usage(source_usage)

    print("Saved V5.2 results to:", METRICS_DIR)
