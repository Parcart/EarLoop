...
CommandName = Literal[
    ...
    "list_audio_devices",
    "list_profiles",
    "get_audio_bridge_health",
    "get_audio_status",
    "get_runtime_profile_status",
    "get_session_preview_status",
    "get_domain_worker_health",
    "apply_engine_config",
    ...
    "start_session",
    "generate_next_pair",
    "shutdown_audio_bridge",
    "shutdown_domain_worker",
]
...
class EngineAudioConfig:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "EngineAudioConfig":
        return cls(
            capture_source_type=str(payload.get("captureSourceType", "input")),
            capture_device_id=str(payload.get("captureDeviceId", payload["inputDeviceId"])),
            output_device_id=str(payload["outputDeviceId"]),
            sample_rate=str(payload["sampleRate"]),
...
class EngineConfig:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "EngineConfig":
        return cls(
            audio=EngineAudioConfig.from_dict(payload["audio"]),
            active_profile_id=str(payload["defaults"]["activeProfileId"]),
            processing_enabled=bool(payload.get("runtime", {}).get("processingEnabled", True)),
        )
...
class EngineStatus:
    ...
    domain_worker_status: dict[str, Any] = field(default_factory=dict)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "domainWorkerStatus": dict(self.domain_worker_status),
