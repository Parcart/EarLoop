...
class AudioRuntimeController:
    def __init__(self) -> None:
        ...
        self._log_runtime_identity(context="controller_init")
    ...
    def _runtime_identity_snapshot(self) -> dict[str, Any]:
        frozen = bool(getattr(sys, "frozen", False))
        soundcard_spec = find_spec("soundcard")
        soundcard_module = sys.modules.get("soundcard")
        return {
            "sysExecutable": sys.executable,
            "frozen": frozen,
            "frozenMode": "pyinstaller" if frozen and hasattr(sys, "_MEIPASS") else ("frozen" if frozen else "python"),
            "soundcardModuleFile": getattr(soundcard_module, "__file__", None),
            "soundcardModuleOrigin": getattr(soundcard_spec, "origin", None),
        }
    ...
    def _log_runtime_identity(
        ...
        *,
        ...
        capture_source_type: str | None = None,
        ...
        self._logger.info(
            "runtime identity [%s]: exe=%s frozen=%s frozen_mode=%s capture_strategy=%s capture_source_type=%s capture_device_id=%s capture_label=%s output_device_id=%s output_label=%s loopback_endpoint_id=%s soundcard_module=%s soundcard_origin=%s",
            context,
            snapshot["sysExecutable"],
            snapshot["frozen"],
            snapshot["frozenMode"],
            capture_strategy,
            capture_source_type,
            ...
            snapshot["soundcardModuleFile"],
            snapshot["soundcardModuleOrigin"],
        )
    ...
    def apply_engine_config(self, config: EngineConfig) -> AudioStatusSnapshot:
        ...
        try:
            ...
            self._log_runtime_identity(
                context="apply_engine_config",
                capture_strategy=resolved_route.capture_strategy,
                capture_source_type=resolved_route.capture_source_type,
                capture_device_id=resolved_route.capture_config_device_id,
                capture_label=resolved_route.capture_label,
                output_device_id=resolved_route.output_device_id,
                output_label=resolved_route.output_label,
                loopback_endpoint_id=resolved_route.capture_runtime_device_id if resolved_route.capture_strategy == NATIVE_LOOPBACK_STRATEGY else None,
            )
    ...
    def shutdown(self) -> None:
        ...
        try:
            self._engine.stop()
            self._logger.info("audio runtime engine stopped")
        except Exception as exc:
            self._logger.warning("audio runtime shutdown failed: %s", exc)
        finally:
            self._engine = None
